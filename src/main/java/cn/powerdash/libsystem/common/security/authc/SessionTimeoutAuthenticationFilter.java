...
import cn.powerdash.libsystem.common.constant.ResultCode;
...
public class SessionTimeoutAuthenticationFilter extends FormAuthenticationFilter {
    ...
    protected void saveRequestAndRedirectToLogin(ServletRequest request, ServletResponse response) throws IOException {
        ...
        if (session != null) {
            LOGGER.debug(
                    ...
                    session.getId(), session.isNew(), DateFormatUtils.format(session.getCreationTime(), DATE_FORMAT),
    ...
    public boolean onPreHandle(ServletRequest request, ServletResponse response, Object mappedValue) throws Exception { // NOSONAR
        ...
        if (SecurityContext.isAuthenticated()) {
            MDC.put(ApplicationConstant.LOG_USER_NAME, SecurityContext.getCurrentUser().getUsername());
        }
        String sessionId = ((HttpServletRequest) request).getSession(false).getId();
        if (sessionId != null) {
            MDC.put(ApplicationConstant.LOG_SESSION_ID,
                    sessionId.substring(sessionId.length() - SESSION_ID_SECTION_LEN));
        }
        return res;
    }
